...
def draw_single_qubit_bloch_time_evolution(
    amplitudes, ax=None, fig=None, figsize=(8, 8), dpi=100, interval=0.3, filename=None, **kwargs
):
    """
...
def _main(graph=0):
    ...
    """
    if not graph:
        amp = np.array([-0.29426025 + 0.64297038j, -0.29426025 - 0.64297038j])
        v = decompose(amp)
        print(v)
        ...
        w = np.matrix([[1, 0]])
        h = 1.0 / np.sqrt(2.0) * np.matrix([[1, 1], [1, -1]])
        ang = np.pi / 8
        rz = np.matrix([[np.exp(-0.5 * 1j * ang), 0], [0, np.exp(0.5 * 1j * ang)]])
        amps = []
        amps.append(w)
        w = w * h
        amps.append(w)
        ...
        fig, _, ims = draw_single_qubit_bloch_time_evolution(
            amps,
            fig=fig,
            interval=0.1,
            filename='bloch.gif',
