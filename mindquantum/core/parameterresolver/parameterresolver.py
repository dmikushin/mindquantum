...
class ParameterResolver:
    ...
    def get_cpp_obj(self):
        ...
        const = self.const
        cpp = mb.parameter_resolver(
            self.data,
            self.no_grad_parameters,
            set(self.params_name) - self.no_grad_parameters,
            self.encoder_parameters,
            set(self.params_name) - self.encoder_parameters,
            const,
            is_const,
        )
        self.const = const
    ...
    def __iadd__(self, other):
        ...
        if isinstance(other, ParameterResolver):
            ...
            for k, v in other.data.items():
                if k in self.data:
                    if (
                        k in self.no_grad_parameters
                        and k not in other.no_grad_parameters
                        or k in other.no_grad_parameters
                        and k not in self.no_grad_parameters
                    ):
                        raise RuntimeError(f"gradient property of parameter {k} conflict.")
                    if (
                        k in self.encoder_parameters
                        and k not in other.encoder_parameters
                        or k in other.encoder_parameters
                        and k not in self.encoder_parameters
                    ):
    ...
    def update(self, other):
        ...
        for k, v in other.items():
            if (
                k in other.no_grad_parameters
                and (k in self.data and k not in self.no_grad_parameters)
                or (k not in other.no_grad_parameters and k in self.no_grad_parameters)
            ):
                raise ValueError(
                    f"Parameter conflict, {k} require grad in some parameter resolver and not \
require grad in other parameter resolver."
                )
            if (
                k in other.encoder_parameters
                and (k in self.data and k not in self.encoder_parameters)
                or (k not in other.encoder_parameters and k in self.encoder_parameters)
            ):
                raise ValueError(
                    f"Parameter conflict, {k} is encoder parameter in some parameter resolver and is not \
