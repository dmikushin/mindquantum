...
namespace mindquantum::core {
    CppCore::CppCore()
        : simulator_backend_(false), has_new_operations_(false), sim_(nullptr), output_stream(&(std::cout)) {
    }
            ...
            circuit_manager_.foreach_instruction(
                [&](const instruction_t& inst) {
                    ...
                    if (inst.is_one<ops::Measure>()) {
                        ...
                        return;
                    } else if (inst.is_one<td::Op::X, td::Op::Y, td::Op::Z, td::Op::S, td::Op::Sdg, td::Op::T,
                                           td::Op::Tdg, td::Op::P, td::Op::H, td::Op::Rx, td::Op::Ry, td::Op::Rz,
                                           td::Op::Sx, ops::Ph>()) {
        ...
        for (auto& veng: engine_list_) {
            if (std::holds_alternative<cengines::cpp::LocalOptimizer>(veng)) {
                ...
                circuit_manager_.transform(
                    [&](const auto& circuit) { return cpp_decomposer.decompose_circuit(circuit); });
