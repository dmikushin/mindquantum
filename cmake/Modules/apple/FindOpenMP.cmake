    ...
    elseif(("${CMAKE_${LANG}_COMPILER_ID}" STREQUAL "AppleClang" AND CMAKE_${LANG}_COMPILER_VERSION
      ...
      if(OpenMP_libomp_LIBRARY)
        ...
        if(NOT OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG})
          # Retry with the LINK_DIRECTORIES path set

          get_filename_component(_omp_dir ${OpenMP_libomp_LIBRARY} DIRECTORY)
          set(OpenMP_libomp_LIBRARY_DIR
              ${_omp_dir}
              PARENT_SCOPE)
        ...
        if(OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG})
          ...
          set("${OPENMP_LIB_NAMES_VAR}"
              "libomp"
              PARENT_SCOPE)
          if(OpenMP_libomp_LIBRARY_DIR)
            set(OpenMP_libomp_LIBRARY_DIR
                "${OpenMP_libomp_LIBRARY_DIR}"
                PARENT_SCOPE)
          endif()
          break()
        endif()
  ...
  if(CMAKE_${LANG}_COMPILER_LOADED)
    if(NOT DEFINED OpenMP_${LANG}_FLAGS
      ...
      if(OpenMP_libomp_LIBRARY_DIR)
        set(OpenMP_libomp_LIBRARY_DIR
            "${OpenMP_libomp_LIBRARY_DIR}"
            CACHE STRING "Directory to OpenMP library directory" FORCE)
        mark_as_advanced(OpenMP_libomp_LIBRARY_DIR)
      endif()
...
foreach(LANG IN LISTS OpenMP_FINDLIST)
  if(CMAKE_${LANG}_COMPILER_LOADED)
    ...
    if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.17)
      set(_names_mismatch NAME_MISMATCHED)
    endif()

    find_package_handle_standard_args(
      OpenMP_${LANG} ${_names_mismatch}
    ...
    if(OpenMP_${LANG}_FOUND)
      ...
      if(OpenMP_libomp_LIBRARY_DIR)
        # NB: As long as this file is for MacOS related workaround, we should be ok with -LXXX ...
        set_property(TARGET OpenMP::OpenMP_${LANG} PROPERTY INTERFACE_LINK_OPTIONS "-L${OpenMP_libomp_LIBRARY_DIR}")
      endif()
